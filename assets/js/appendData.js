...
function appendGrid() {
    ...
    for (let i = 0; i < projectsData.length; i++) {
        if (projectsData[i].vorschaubild != "" && projectsData[i].raum == roomname) {
            item += `
        ...
        } else if(projectsData[i].vorschaubild == "" && projectsData[i].raum == roomname){
...
function openModal(clicked_id) {
    ...
    if ( project.id === clicked_id) {
        ...
        author.innerText = "Studierende: " + String(project.urheber).replace(/,/g, ", ");
        ...
        for (let i = 0; i < video.length; i++) {
            videoBlock += `<iframe id="video-player"
                                src="${String(video[i]).replace('watch?v=', 'embed/')}">
                            </iframe>`;
        }
        ...
        console.log("HTML-Item-ID: " + clicked_id + " Array-ID: " + project.id + " Array-Titel: " + project.titel + " Array-Urheber: " + project.urheber);
    }
    else{
        console.log(false)
        return
